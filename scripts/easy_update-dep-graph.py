...
class ExtsList(object):
    ...
    def __init__(self, file_name, add_packages, package, verbose):
        ...
        self.dep_graph = []  # [pkg_name, [dependancies]]
        self.prolog = '## remove ##\n'
        self.ptr_head = 0
        self.indent_n = 4
    ...
    def check_package(self, pkg):
        ...
        self.exts_processed.append(pkg)
        self.dep_graph.append([pkg[0], depends])
        self.ext_counter += 1
        if self.verbose > 0:
            if len(pkg) < 4:
                print("Error:"),
    ...
    def print_dep_graph(self):
        """output dependancy graph for modules from the exts_list
           output is written in DOT file format"""
        graph = open(self.pkg_name + ".dot", 'w')
        self.graph.write('digraph %s-%s {' % (self.name, self.version))
        for pkg, depends in self.dep-graph:
           self.graph.write('%s' % pkg)
           for dep in depends:
               graph.write('-> %s' % dep)
           graph.write('\n')
        graph.write('}')
