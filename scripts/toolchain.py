...
class Toolchain():
    ...
    @classmethod
    def tc_trim(cls, suffix):
        """ return <suffix> with out the toolchain text
        Remove toolchain and possible version suffix like '-CUDA-X.X'
        input: '0.58.1-foss-2023a' output: '0.58.1'
        """
        for tc_gen in cls.toolchains:
            for tc_name in tc_gen:
                pos = suffix.find(tc_name)
                if pos:
                   return suffix[:pos-1]

    def tc_filter(cls, mod_name):
