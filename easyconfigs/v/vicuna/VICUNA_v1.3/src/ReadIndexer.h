...
/*
 * Assumption: one read per line in the file
 *
 * Testing code:
 ivec_t rIds;
 rIds.push_back(4);
 ...
 rIndexer.getReads (std::back_inserter(reads), rIds.begin(), rIds.end(),
 myPara.inputFiles);
 ...
 */
/*
	Template for handling reads in batches:

	// process reads in batches
	std::vector<read_t> reads;
	int counter = 0;
	ivec_t rIDs;
	for (int i = 0; i < rIndexer.size(); ++ i) {
		rIDs.push_back(i);
		counter ++;
...
typedef struct ReadEntry {
	ReadEntry(int id, const std::string& h, std::string r, std::string s):
	ID (id), header(h), read(r), score(s){}
	ReadEntry(int id): ID(id) {}
	ReadEntry(){}
	bool paired;
	int ID;
	std::string header;
...
// sorting read_t
inline bool operator<(const read_t& lhs, const read_t& rhs) {
...
public:
	...
	/*
	 * input is a container type (set, vector), consists of sorted readIDs
	 */
	template<typename oIter, typename iIter>
	void getReads(oIter output, iIter first, iIter last) const {
...
private:
	...
	void seekRead(read_t& entry, uint64_t offset, std::ifstream* fH) const {
		std::string dummie;
		if (!fH->good()) fH->clear();
	...
	void getInputFilelist(const std::string& pFqDir, const std::string& npFqDir,
			const std::string& pFaDir, const std::string& npFaDir) {
		...
		if (pFqDir.empty() && npFqDir.empty() && pFaDir.empty()
			...
			strvec_t fileNms;
			if (!pFqDir.empty()) {
				if (xny::dir_list(pFqDir, std::back_inserter(fileNms)) == false) {
					...
					for (int i = 0; i < (int) fileNms.size(); ++i) {
						...
						if (xny::get_suffix(fileNms[i], true, ".").compare("fq") == 0
						 || xny::get_suffix(fileNms[i], true, ".").compare("fastq") == 0){
			...
			fileNms.clear();
			if (!pFaDir.empty()) {
				if (xny::dir_list(pFaDir, std::back_inserter(fileNms)) == false) {
					...
					for (int i = 0; i < (int) fileNms.size(); ++i) {
						if (fileNms[i].compare(".") == 0
								|| fileNms[i].compare("..") == 0)
							continue;
						if (xny::get_suffix(fileNms[i], true, ".").compare("fa") == 0
						 || xny::get_suffix(fileNms[i], true, ".").compare("fasta") == 0){
			...
			fileNms.clear();
			if (!npFaDir.empty()) {
				if (xny::dir_list(npFaDir, std::back_inserter(fileNms)) == false) {
					std::cout << "can't read from dir: " << npFaDir << "\n";
					exit(1);
				} else {
					for (int i = 0; i < (int) fileNms.size(); ++i) {
